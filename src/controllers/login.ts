...
} from "../helpers/google-verify-token";
...
export const correoRecuperacion = async (req: any, res: any) => {
  ...
  try {
    ...
    let responseEmail = await fetch(process.env.URL_SEND_EMAIL, {
      method: "POST",
      body: JSON.stringify(dataMail),
      headers: { "Content-Type": "application/json" },
    });
    let responseData = await responseEmail.json();

    if (![200, 201].includes(responseEmail.status)) {
      res.status(500).json({
...
export const saveNewPass = async (req: any, res: any) => {
  ...
  try {
    ...
    let result = await loginProvider.updatePass(
      dataToken.usuario.login,
      body.pass
    );
    res.status(200).json({
      error: false,

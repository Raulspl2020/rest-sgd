...
import {
  ...
  consultaFacturaBanco,
  consultaFacturaCliente,
  consultaPagoFacturaCliente,
  consultarPagoFactura,
  eliminarFacturaRef,
  existeDetPago,
  existeDetPagoWhere,
  insertPagoMR5,
  reversarPagoyDetalle,
} from "../provider/factura_provider";
...
import {
  eliminarFacturaSysApolo,
  registroFacturaSysApolo,
} from "./sysapolo/factura";
    ...
    if (
      ...
      if (resultObjectDB != false) {
        ...
        if (categoria_id == 1) {
          ...
          let periodo = await getFechasPeriodo(
          ...
          if (fechaActual.getTime() > finOrdianria.getTime()) {
            fechaLimitePago = format(
              new Date(periodo.fec_fin_matextraord),
              "DD/MM/YYYY"
            );
          } else if (
            fechaActual.getTime() <= finOrdianria.getTime() &&
            fechaActual.getTime() >= iniOrdianria.getTime()
          ) {
            fechaLimitePago = format(
              new Date(periodo.fec_fin_matordinaria),
              "DD/MM/YYYY"
            );
          }
        }
        ...
        if (resultObjectDB.data[0].estado_id == 1) {
          let resultDBPago = await existeDetPago(
            Referencia_pago,
            resultObjectDB.total
          );
          if (resultDBPago != false) {
            ...
            responseData.Info_Adicional =
              "La factura " + Referencia_pago + " ya ha sido pagada";
          }
        }
      } else {
        responseData.Codigo_Estado = "1";
        responseData.Descripción_estado =
          "Factura no disponible para pago / Cliente no Existe";
        responseData.Info_Adicional =
          "No se encontro la factura " + Referencia_pago;
      }
  ...
  try {
    if (
      ...
      if (resultObjectDB.data[0].estado_id == 1) {
        ...
        if (resultDBPago != false) {
          throw new Error(
            "La factura " + Referencia_pago + " ya se encuentra pagada"
          );
        }
      }
      ...
      detPago.push({
        ...
        fecha: format(
          parse(Fecha_pago + " " + horaActual, "DD/MM/YYYY HH:mm:ss"),
          "YYYY-MM-DD HH:mm:ss"
        ),
      ...
      let resultUpdatePago = await actualizarPagoyDetalle(
        Referencia_pago,
        tPago,
        detPago
      );
      ...
      if (resultUpdatePago != false) {
        ...
        responseData.Descripcion =
          "Se realizó exitosamente la actualización del pago.";
        ...
        responseData.Descripción_estado =
          "No se pudo realizar la actualización del pago.";
      }
  ...
  try {
    if (
      ...
      let tPago: any = {
        ...
        fecha_reverso: format(
          parse(Fecha_reverso + " " + horaActual, "DD/MM/YYYY HH:mm:ss"),
          "YYYY-MM-DD HH:mm:ss"
        ),
        valor_reverso: Valor_pagado,
      };

      let resultUpdatePago = await reversarPagoyDetalle(
        Referencia_pago,
        tPago,
        detPago
      );
      ...
      if (resultUpdatePago != false) {
        ...
        responseData.Severidad = "I";
        responseData.Descripcion =
          "Se realizó exitosamente el reverso del pago";
...
export const consultaEstadoFactura = async (req: any, res: any) => {
  ...
  try {
    let total_a_pagarString: any = moneda
      .format(0, { locale: "es-CO" })
      .replace("$", "")
      .trim();
    ...
    for (const factura of facturas) {
      ...
      for (const row of resultDB) {
        if (factura.id == row._id) {
          let subtotal =
            row.valor_unidad -
            row.valor_unidad * row.descuento +
            row.valor_unidad * row.aumento;
          det_factua.push({
      ...
      factura.total_a_pagar_s = moneda
...
export const getDataDetalleFacturaById = async (idFactura: any) => {
  ...
  try {
    ...
    if (dataConceptos.length > 0) {
      for (const pago of dataConceptos) {
        ...
        factura = {
          ...
          fecha: pago.fecha,
          verify: pago.sysapolo_verify ?? '0',
          email_send: pago.email_send ?? '0',
        };
        ...
        let subtotal =
          subtotal1 - subtotal1 * pago.descuento + subtotal1 * pago.aumento;
        ...
        det_factura.push({
          ...
          valor_unidad: moneda
            .format(pago.valor_unidad, { locale: "es-CO" })
            .replace("$", "")
            .trim(),
          cantidad: pago.cantidad,
          subtotal: moneda
            .format(subtotal, { locale: "es-CO" })
            .replace("$", "")
            .trim(),
        });
      }
    ...
    for (const pago of dataPagos) {
      ...
      pago.fecha = format(pago.fecha, "DD-MM-YYYY hh:mm:ss A");
      pago.nombre_banco =
        pago.nombre_banco == null ? "NO APLICA" : pago.nombre_banco;
      pago.codigo_transaccion =
        pago.codigo_transaccion == null ? "NO APLICA" : pago.codigo_transaccion;
      pago.ticketID = pago.ticketID == null ? "NO APLICA" : pago.ticketID;
      pago.numero_tarjeta =
        pago.numero_tarjeta == null ? "NO APLICA" : pago.numero_tarjeta;
      pago.franquicia = pago.franquicia == null ? "NO APLICA" : pago.franquicia;
      pago.cod_aprobacion =
        pago.cod_aprobacion == null ? "NO APLICA" : pago.cod_aprobacion;
      pago.num_recibido =
        pago.num_recibido == null ? "NO APLICA" : pago.num_recibido;
    ...
    if (req.files && req.files.archivo) {
      ...
      if (pagoSinRegistrar.length > 0) {
        ...
        if (insertDB) {
          ...
          const contenidoArchivo = fs.readFileSync(archivo.tempFilePath, {
            encoding: "utf8",
          });
...
const leerLienas = async (
  file2: readline.Interface
): Promise<DetallePago[]> => {
  let pagos: DetallePago[] = [];
  let fecha = "";
